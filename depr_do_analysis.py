...
def compute_and_plot_pca_ne(df, station_name):
	# Filter data
	date_filtered_df = df[
		(df['date'] >= datetime.strptime("26.12.2004", "%d.%m.%Y")) &
		(df['date'] <= datetime.strptime("26.12.2005", "%d.%m.%Y"))
	]

	# Select features for PCA
	X = date_filtered_df[['d_north_mm', 'd_east_mm']].values

	# Apply PCA
	pca = PCA(n_components=2)
	pca.fit(X)

	# Transform the full dataset (you can also transform just filtered if needed)
	components = pca.transform(df[['d_north_mm', 'd_east_mm']])

	# Assign new columns properly — needs .loc or .assign and correct column labels
