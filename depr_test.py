...
station_name = "PHKT"
columntobecalced = 'pc1'
#df = earthquake_correction.remove_earthquake_column(df, station_name, '2012-04-11','d_north_mm')
#df = earthquake_correction.remove_earthquake_column(df, station_name, '2012-04-11','d_east_mm')

#df = earthquake_correction.remove_earthquake_column(df, station_name, 'March 28, 2005','d_north_mm')
#df = earthquake_correction.remove_earthquake_column(df, station_name, 'March 28, 2005','d_east_mm')
print('at earthquake:',df[df['date'] == pd.to_datetime('2004-12-26')][['d_north_mm','d_east_mm']])

closest_values = get_closest_values(
	df,
	target_date=datetime(2004, 12, 26),
	columns=['d_north_mm', 'd_east_mm'],
	direction='before'  # or 'after' or 'both'
)
print("closest values", closest_values)


df, pca = PCA_fit.compute_and_apply_pca_ne(df, station_name)
...
print("curve fitting thingies",popt, pcov)
...
plot_column_for_station(df, station_name, columntobecalced,v =v, popt=popt, ref_date=ref_date)
...
def find_height_reintercept_date(popt, v, ref_date, height_intercept):
	...
	func_at_end = func(365 * 1000)  # Change to 100,000 years
